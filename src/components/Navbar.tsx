...
import { Dumbbell, LogOut, LayoutDashboard, User, Calculator, ListChecks, Activity, MessageCircle, History, Youtube } from "lucide-react";
...
const Navbar = () => {
  ...
  const navItems = [
    ...
    { path: "/exercise-tutorials", icon: Youtube, label: "Tutorials" },
            ...
            {navItems.map((item) => {
              ...
              return (
                <Link
                  key={item.path}
                  to={item.path}
                  className={`flex items-center gap-2 px-3 py-2 rounded-md text-sm transition-colors ${location.pathname === item.path
